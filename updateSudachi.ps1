...
# - $targetFile: The name of the target executable file (e.g., "sudachi.exe").
...
if ($Help) {
    @"
Usage: .\updateSudachi.ps1 [-EnableDebug] [-Verbose] [-Help]
...
$targetFile = "sudachi.exe"  # Change this to your target file name
$targetFolder = Join-Path -Path $env:USERPROFILE -ChildPath "Launchbox\Systems\sudachi"  # Change this to your target folder path
...
try {
    ...
    $response = Invoke-RestMethod -Uri "https://api.github.com/repos/emuplace/sudachi.emuplace.app/releases/latest" -Headers $headers -Timeout 2
    ...
    if ($latestVersion -ne $formattedCurrentVersion) {
        ...
        # Filter the download URL for the win64.7z file
        $downloadUrl = $response.assets | Where-Object { $_.name -like "*windows*.7z" } | Select-Object -ExpandProperty browser_download_url
        
        # Download the file
        Write-DebugMessage "Downloading from $downloadUrl..."
        $outputFile = "sudachi-binaries-win64.zip"
    ...
    $errorMessage = "Error: An error occurred while fetching the latest release information from GitHub: $_"
    Write-DebugMessage $errorMessage
