...
$targetFile = "rpcs3.exe"  # Change this to your target file name
$targetFolder = Join-Path -Path $env:USERPROFILE -ChildPath "Launchbox\Systems\rpcs3"  # Change this to your target folder path
...
try {
    ...
    $response = Invoke-RestMethod -Uri "https://api.github.com/repos/RPCS3/rpcs3-binaries-win/releases/latest" -Headers $headers -Timeout 2
    ...
    if ($latestVersion -ne $formattedCurrentVersion) {
        ...
        $downloadUrl = $response.assets | Where-Object { $_.name -like "*win64.7z" } | Select-Object -ExpandProperty browser_download_url
        
        # Download the file
        Write-DebugMessage "Downloading from $downloadUrl..."
        $outputFile = "rpcs3-binaries-win64.zip"
