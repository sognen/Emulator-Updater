...
if ($Help) {
    @"
Usage: .\updateLime3ds.ps1 [-EnableDebug] [-Verbose] [-Help]
...
$targetFile = "lime3ds.exe"  # Change this to your target file name
$targetFolder = Join-Path -Path $env:USERPROFILE -ChildPath "Launchbox\Systems\lime3ds"  # Change this to your target folder path
...
try {
    ...
    $response = Invoke-RestMethod -Uri "https://api.github.com/repos/Lime3DS/Lime3DS/releases/latest" -Headers $headers -Timeout 2

    # Extract only the version number
    $latestVersion = $response.name -replace 'Lime3DS ', ''  # Remove any prefix (e.g., "Lime3DS ")
    ...
    if ($latestVersion -ne $currentVersion) {
        ...
        $downloadUrl = $response.assets | Where-Object { $_.name -like "*windows-msvc.zip" } | Select-Object -ExpandProperty browser_download_url
        
        # Download the file
        Write-DebugMessage "Downloading from $downloadUrl..."
        $outputFile = "lime3ds-binaries-win64.zip"
        ...
        $sourceFolder = Join-Path -Path $targetFolder -ChildPath "lime3ds-$latestVersion-windows-msvc"
        Write-Verbose "******* $sourceFolder" -Verbose        
