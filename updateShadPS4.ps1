...
# - $targetFile: The name of the target executable file (e.g., "shadps4.exe").
...
$targetFile = "shadPS4.exe"  # Change this to your target file name
$targetFolder = Join-Path -Path $env:USERPROFILE -ChildPath "Launchbox\Systems\shadps4"  # Change this to your target folder path
...
try {
    ...
    $response = Invoke-RestMethod -Uri "https://api.github.com/repos/shadps4-emu/shadPS4/releases/latest" -Headers $headers -Timeout 2
    ...
    if ($latestVersion -ne $formattedCurrentVersion) {
        ...
        $downloadUrl = $response.assets | Where-Object { $_.name -like "*win64-qt*.zip" } | Select-Object -ExpandProperty browser_download_url
        

        # Download the file
        Write-DebugMessage "Downloading from $downloadUrl..."
        $outputFile = "shadps4-binaries-win64.zip"
